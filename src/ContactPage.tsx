...
const categories = ['SUBMISSION', 'BUSINESS', 'OTHERS'] as const;

const copy = {
  zh: {
    name: '称呼', email: '联系邮箱', subject: '主题',
    work: '作品链接（必填）', link: '相关链接（选填）', message: '正文', hint: '可附链接访问说明',
  ...
  en: {
    ...
    work: 'Track link (required)', link: 'Related link (optional)', message: 'Message', hint: 'Include access details if needed',
  ...
  ja: {
    ...
    work: '作品リンク（必須）', link: '関連リンク（任意）', message: '本文', hint: '必要に応じてアクセス方法をご記入ください',
...
export function ContactPage({ locale }: { locale: Locale }) {
  ...
  // Keep links to the previous categories within the merged OTHERS category.
  const activeCategory = requestedCategory === 'other' || requestedCategory === 'questions' ? 'others' : requestedCategory;
  const categoryIndex = categories.findIndex(category => category.toLowerCase() === activeCategory);
  ...
  return <>
    <div className="contact-viewport" ref={viewportRef}><div className="contact-page">
      ...
      <section ref={editorRef} className="editor" aria-labelledby="editor-title" onInput={event => {
        ...
        <label className="field">{kind === 0 ? text.work : text.link}<input id="work-link" type="url" placeholder="https://" required={kind === 0} /></label>
